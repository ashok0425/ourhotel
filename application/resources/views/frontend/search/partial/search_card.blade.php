...
<div class="row custom-border-radius-20 m-0 shadow-sm p-3 mb-3 bg-white ">
    <div class="col-12 col-sm-9 col-md-9">
        <div class="row">
            <div class="col-12 col-sm-5 col-md-5 nsnhotelsphoto pr-0">
                <div id=""
                    class="mr-2 ${place.couple_friendly===1?'display_after':'' }">

                            <div class="nsnhotelsimagesliderbox ">
                                <img src="${ place.thumbnail?place.thumbnail:'https://via.placeholder.com/1920x1200?text=NSN' }}" alt="{{ $place->name }}" class="img-responsive pr-2"
        ...
        <div class="my-2 mb-3 ${place.couple_friendly===1?'d-block':'d-none' }">
            ...
            <div class="text-success custom-fw-600 custom-fs-16"><i class="fas fa-check-circle fs-2z"></i><span></span> Pay At Hotel</div>
                ...
                <div>
                    <ul class="">
                            @foreach ($item->amenitiesList as $key => $item)
    ...
    <div class="col-12 col-sm-3 col-md-3 nsnhotelssearchdatarightarea">
        <div class="nsnhotelsreviews text-right">
            <div class="product__rating d-none d-md-block">
                <div class="top">
                    <small>
                            {{ number_format($place->avg_rating, 0) }}/5
                    </small>
                    <br>
                    <svg viewBox="0 0 9 10" xmlns="http://www.w3.org/2000/svg">
                        <path
        ...
        <div class="nsnhotelspernightpricevalue">
            ...
            {{    $place->price}}
                ...
                @if (isset($place->discount_percent))
                    <p class="my-0 py-0 line-height-1 mb-2">
                        <s class="custom-fs-14 custom-fw-600 text-danger">
                            {{$before_price }}  </s>
                        <span class="custom-fs-16 custom-fw-600 custom-text-primary"> {{ $place->price }}% off</span>
                    </p>
                @endif

                    @if (isset($place->hourlyprice))
                    <p class="my-0 py-0 line-height-1 ">
                        <span class=" ">
                            <span class="custom-fs-16 custom-fw-600">{{$place->hourlyprice}} + <span class="custom-fs-12">GST / Per 3
                                Hours</span></span>
                        </span><span>
                        </p>
                    @endif
 ...
 <div class="d-flex justify-md-content-between justify-content-around mt-3">
    <div class="">
        <button class="px-3 py-2 btn custom-border-radius-20 custom-bg-primary hover-on-white" type="submit"><a href="{{ $place->route }}" class="custom-fs-14 custom-text-white custom-fw-600">Book
                Now</a></button>
    </div>
    <div class="">
